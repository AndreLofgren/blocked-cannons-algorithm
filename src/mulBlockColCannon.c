...
void fillMatrixCol(double *M, int n);
void mulRR(double *C, double *A, double *B, int n);
void mul(double *C, double *A, double *B, int n, int m);
...
void printMatrixCol(double *M, int n);
double timer();
...
int main(int argc, char *argv[]) {
    ...
    if (grid_rank == root) {
        for (i = 0; i < py; i++) {
            for (j = 0; j < px; j++) {
                ...
                indexA = i*n*ny + j*nx;
                indexB = j*n*ny + i*nx;
                MPI_Isend(&A[indexA], 1, Block, sendA, 0, GRID, &request[0]);
                MPI_Isend(&B[indexB], 1, Block, sendB, 1, GRID, &request[1]);
            }
        }
    }
    ...
    for (k = 0; k < px; k++) {
        ...
        mul(subMatC, subMatA, subMatB, nx, BLOCK_SIZE);
